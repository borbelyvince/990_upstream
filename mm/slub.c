...
#ifdef CONFIG_KDP_CRED
	if (rkp_cred_enable && s->cred_type) {
...
def_alloc:
	...
	if (kdp_cmd) {
		...
		for(sc = 0; sc < (1 << oo_order(oo)); sc++) {
			uh_call(UH_APP_KDP, kdp_cmd, va_page, 0, 0, 0);
			va_page += PAGE_SIZE;
		}
	}
#endif
...
int rkp_from_vfsmnt_cache(unsigned long addr)
{
	...
	s = page->slab_cache;
	if(s && s->cred_type == KDP_VFSMNT_JAR) {
		return 1;
	}
	return 0;
}
...
{
	slab_lock(page);
	BUG_ON(!validate_slab(s, page, map));
	slab_unlock(page);
}
